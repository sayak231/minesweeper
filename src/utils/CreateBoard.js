export default function createBoard(row, col, bombs) {
  ...
  for (let i = 0; i < row; i++) {
    for (let j = 0; j < col; j++) {
      ...
      // Top Right
      if (i > 0 && j < col - 1 && board[i - 1][j + 1].value === "💣") {
        board[i][j].value++;
      }

      // Right
      if (j < col - 1 && board[i][j + 1].value === "💣") {
        board[i][j].value++;
      }

      // Botoom Right
      if (i < row - 1 && j < col - 1 && board[i + 1][j + 1].value === "💣") {
        board[i][j].value++;
      }

      // Bottom
      if (i < row - 1 && board[i + 1][j].value === "💣") {
        board[i][j].value++;
      }

      // Bottom Left
      if (i < row - 1 && j > 0 && board[i + 1][j - 1].value === "💣") {
        board[i][j].value++;
      }
      ...
      // Top Left
      if (i > 0 && j > 0 && board[i - 1][j - 1].value === "💣") {
        board[i][j].value++;
      }
    }
  }
